//src\pages\ShippingForm\ShippingForm.jsx
// ShippingForm: טופס פרטי מסירה לזוכה — בוחר שיטת מסירה לפי המוכר (משלוח/איסוף), ולידציה לנייד, טעינת כתובת שמורה, הצגת כתובת איסוף, סיכום ואישור לפני שליחה, שליחה לשרת עם אפשרות לשמור כתובת/טלפון לפרופיל.
...
function ShippingForm() {
  ...
  const options =
    phonePrefix && !knownPrefixes.includes(phonePrefix)
  ...
  useEffect(() => {
    (async () => {
      // 1) נסה מה-Context
      const fromCtx =
        parseIlMobile(user?.phone) || parseLocalIlMobile(user?.phone);
      if (fromCtx) {
        setPhonePrefix(fromCtx.prefix);
        setPhoneNumber(fromCtx.number);
        setFormData((prev) => ({
          ...prev,
          phone: fromCtx.prefix + fromCtx.number,
        }));
        return;
      }
...
async function savePhoneOnly(fullPhone) {
  ...
  function openSavePhoneOnlyModal(fullPhone) {
    showModal({
      title: "שמירת טלפון",
      message:
        "נקלטה בקשתך לאסוף עצמאית את המוצר, לצפייה בפרטי כתובת המוכר גש לדף ההצעות שלי\n האם לשמור את הטלפון הזה בפרופיל שלך?",
      cancelText: "לא, תודה",
      confirmText: "כן, שמור",
      hideClose: true,
      disableBackdropClose: true,
  ...
  function formatSummary(deliveryMethod, data) {
    ...
    lines.push(
      `שיטת משלוח: ${
        deliveryMethod === "delivery" ? "משלוח עד הבית" : "איסוף עצמי"
      }`
    );
  ...
  const handleUseSavedAddress = async () => {
    try {
      ...
      if (data.success && data.address) {
        let { city, street, house_number, apartment_number, zip, phone } =
          data.address;
        ...
        const streetsWithSaved =
          street && !baseStreets.includes(street)
            ? [street, ...baseStreets]
            : baseStreets;
                  ...
                  <div className={styles.pickupCard} dir="rtl">
                    ...
                    {Boolean(sellerPickupAddress.house_number) && (
                      <div className={styles.row}>
                        ...
                        <span className={styles.value}>
                          {sellerPickupAddress.house_number}
                        </span>
                      </div>
                    )}
                    {Boolean(sellerPickupAddress.apartment_number) && (
                      <div className={styles.row}>
                        ...
                        <span className={styles.value}>
                          {sellerPickupAddress.apartment_number}
                        </span>
                      </div>
                    )}
